...
import { getDynamicPages, updateDynamicPage } from "@/lib/storage"
...
export default function EditPageClient({ params }: EditPageClientProps) {
  ...
  useEffect(() => {
    const loadPage = async () => {
      try {
        ...
        toast({
          title: "خطأ",
          description: "خطأ في تحميل الصفحة",
          variant: "destructive",
        })
      } finally {
        setLoading(false)
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await updateDynamicPage(id, updates)
      toast({
        title: "تم الحفظ",
  ...
  if (isPreviewMode) {
    ...
    return (
      <div className="min-h-screen bg-white" dir={activeLanguage === "ar" ? "rtl" : "ltr"}>
        <div className={`fixed top-4 ${activeLanguage === "ar" ? "right-4" : "left-4"} z-50 flex gap-2 shadow-lg`}>
          <Button onClick={() => setIsPreviewMode(false)} variant="default">
            <X className={`h-4 w-4 ${activeLanguage === "ar" ? "ml-2" : "mr-2"}`} />
            {activeLanguage === "ar" ? "إغلاق المعاينة" : "Close Preview"}
          </Button>
          <Button onClick={handleSave} disabled={saving} variant="secondary">
            {saving
              ? (activeLanguage === "ar" ? "جاري الحفظ..." : "Saving...")
              : (activeLanguage === "ar" ? "حفظ الصفحة" : "Save Page")}
          </Button>
        </div>
          ...
          <div className="flex items-center justify-between py-4">
            <div>
              ...
              <p className="text-sm text-slate-600">
                {isAr ? "استخدم محرر البلوكات لتعديل صفحتك" : "Use the block editor to modify your page"}
              </p>
            </div>
