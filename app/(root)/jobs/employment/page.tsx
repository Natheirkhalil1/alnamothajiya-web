...
export default function EmploymentPage() {
  ...
  const selectedJobObject = availableJobs.find(
    (job) => (language === "ar" ? job.title : job.titleEn) === selectedJob
  )
  ...
  const handlePhotoChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      if (file.size <= 2 * 1024 * 1024) {
        ...
        alert(language === "ar" ? "حجم الصورة يجب أن يكون أقل من 2MB" : "Photo size must be less than 2MB")
      }
    }
  }
  ...
  const isStepValid = () => {
    if (currentStep === 1) {
      ...
      const photoValid = !selectedJobObject?.requiresPhoto || photoFile !== null
    ...
    if (currentStep === 3) {
      ...
      // Cover letter validation (if required by selected job)
      const coverLetterValid = !selectedJobObject?.requiresCoverLetter || coverLetter.trim().length > 0

      // Custom fields validation (if any required fields exist)
      const customFieldsValid = !selectedJobObject?.customFields ||
        selectedJobObject.customFields
          .filter((f: any) => f.required)
          .every((f: any) => {
            const response = customFieldResponses.find((r) => r.fieldId === f.id)
            return response && response.value && response.value.trim().length > 0
          })

      const isValid = experienceValid && coverLetterValid && customFieldsValid
      console.log("[v0] Step 3 validation:", isValid, "Experience count:", formData.experience.length, "Cover letter:", coverLetterValid, "Custom fields:", customFieldsValid)
      return isValid
    }
    if (currentStep === 4) {
                ...
                <form onSubmit={handleSubmit} className="space-y-8">
                  {currentStep === 1 && (
                    <div className="space-y-8 animate-in fade-in slide-in-from-right duration-500">
                      ...
                      {/* Personal Photo - Show if job requires it */}
                      {selectedJobObject?.requiresPhoto && (
                        <div className="space-y-3">
                          ...
                          <div className="flex items-center gap-6">
                            <div className="relative">
                              ...
                              <label
                                htmlFor="photo"
                                className="flex flex-col items-center justify-center w-32 h-32 border-3 border-dashed border-primary/30 rounded-full cursor-pointer hover:border-primary hover:bg-primary/5 transition-all duration-300 bg-gradient-to-br from-muted/30 to-muted/10 group overflow-hidden"
                              >
                                {photoPreview ? (
                                  <img
                                    src={photoPreview}
                                  ...
                                  <div className="text-center">
                                    <Camera className="w-8 h-8 text-primary/50 mx-auto mb-1 group-hover:scale-110 transition-transform" />
                                    <p className="text-xs text-muted-foreground">
                                      {language === "ar" ? "اضغط للرفع" : "Click to upload"}
                                    </p>
                                  </div>
                                )}
                              </label>
                            ...
                            <div className="flex-1">
                              <p className="text-sm text-muted-foreground mb-2">
                                {language === "ar"
                                  ? "يرجى رفع صورة شخصية حديثة وواضحة"
                                  : "Please upload a recent and clear personal photo"}
                              </p>
                              <ul className="text-xs text-muted-foreground space-y-1">
                                <li>• {language === "ar" ? "الحد الأقصى للحجم: 2MB" : "Max size: 2MB"}</li>
                                <li>• {language === "ar" ? "الصيغ المدعومة: JPG, PNG" : "Supported formats: JPG, PNG"}</li>
                              </ul>
                              {photoFile && (
                  ...
                  {currentStep === 3 && (
                    <div className="space-y-8 animate-in fade-in slide-in-from-right duration-500">
                      ...
                      {/* Cover Letter - Show if job requires it */}
                      {selectedJobObject?.requiresCoverLetter && (
                        <div className="space-y-3">
                          <Label className="text-lg font-bold text-foreground flex items-center gap-2">
                          ...
                          <Textarea
                            ...
                            placeholder={
                              language === "ar"
                                ? "اكتب خطاب التغطية هنا... اشرح لماذا أنت مناسب لهذه الوظيفة"
                                : "Write your cover letter here... explain why you're a good fit for this position"
                            }
                            rows={6}
                            required
                            className="border-2 hover:border-primary focus:border-primary transition-colors rounded-xl resize-none"
                      ...
                      {/* Custom Fields - Show based on selected job */}
                      {selectedJobObject?.customFields && selectedJobObject.customFields.length > 0 && (
                        <div className="space-y-4 border-t pt-6 mt-6">
                          ...
                          {selectedJobObject.customFields
                            ...
                            .map((field: any) => {
                              const response = customFieldResponses.find((r) => r.fieldId === field.id)
                              const fieldLabel = language === "ar" ? field.labelAr : field.labelEn

                              const updateFieldResponse = (value: string, fileName?: string) => {
                                setCustomFieldResponses((prev) => {
                                  const existing = prev.find((r) => r.fieldId === field.id)
                                  if (existing) {
                                    return prev.map((r) =>
                                      r.fieldId === field.id ? { ...r, value, fileName } : r
                                    )
                                  }
                                  return [...prev, { fieldId: field.id, value, fileName }]
                                })
                              }
                                ...
                                <div key={field.id} className="space-y-2">
                                  ...
                                  {field.type === "text" && (
                                    <Input
                                      ...
                                      onChange={(e) => updateFieldResponse(e.target.value)}
                                      placeholder={language === "ar" ? field.placeholder : field.placeholderEn}
                                  ...
                                  {field.type === "select" && field.options && (
                                    <Select
                                      value={response?.value || ""}
                                      onValueChange={(value) => updateFieldResponse(value)}
                                      required={field.required}
                                    >
                                      <SelectTrigger className="h-12 border-2 hover:border-primary transition-colors rounded-xl">
                                        <SelectValue
                                      ...
                                      <SelectContent>
                                        {field.options.map((option: any, idx: number) => (
                                          <SelectItem
                                            key={idx}
                                            value={language === "ar" ? option.valueAr : option.valueEn}
                                          >
                                    ...
                                    <div className="relative">
                                      <Input
                                        ...
                                        onChange={(e) => {
                                          const file = e.target.files?.[0]
                                          if (file) {
                                            updateFieldResponse(file.name, file.name)
                                          }
                                        }}
                                        required={field.required}
                                        className="h-12 border-2 hover:border-primary focus:border-primary transition-colors rounded-xl pt-2"
                                      />
                                      {response?.fileName && (
                                        <p className="text-sm text-green-600 mt-1">
                                          {language === "ar" ? "تم تحميل:" : "Uploaded:"} {response.fileName}
                                        </p>
                                      )}
                                    </div>
                                  )}
                      ...
                      <div className="space-y-6">
                        ...
                        {coverLetter && (
                          <Card className="p-6 border-2 border-primary/20">
                            ...
                            <p className="text-sm text-foreground bg-muted/30 p-4 rounded-xl whitespace-pre-wrap">
                              {coverLetter}
                            </p>
                          </Card>
                        )}
                        ...
                        {customFieldResponses.length > 0 && selectedJobObject?.customFields && (
                          <Card className="p-6 border-2 border-primary/20">
                            <div className="flex items-center gap-3 mb-4">
                              <FileText className="w-6 h-6 text-primary" />
                              <h4 className="text-xl font-bold">
                            ...
                            <div className="space-y-3">
                              {customFieldResponses.map((response) => {
                                const field = selectedJobObject.customFields?.find(
                                  (f: any) => f.id === response.fieldId
                                )
                                if (!field || !response.value) return null
                                return (
                                  <div key={response.fieldId} className="p-3 bg-muted/30 rounded-lg">
