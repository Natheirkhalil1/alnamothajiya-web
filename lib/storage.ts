...
import { getFromLocalStorage, saveToLocalStorage } from "./storage-adapter"
// FIREBASE: Original Firebase Timestamp import
// import type { Timestamp } from "firebase/firestore"

// FIREBASE: Original Firebase module loader (preserved for future production use)
/*
...
async function loadFirestore() {
  if (!firestoreModule) {
    try {
      ...
      const db = getDb()
      firestoreModule = { getDb: () => db, FIREBASE_COLLECTIONS, ...firestore }
    } catch (error) {
...
async function loadFirebaseAuth() {
  if (!firebaseModule) {
    try {
      ...
      firebaseModule = { getFirebaseAuth: () => authInstance, ...auth }
    } catch (error) {
      console.log("Firebase not available, using localStorage fallback:", error)
      firebaseModule = null
    }
...
console.log("[v0] Storage module initialized - localStorage-only mode")
...
// Collection names for localStorage keys
  ...
  if (!firestoreModule) {
    try {
      ...
      if (typeof window === "undefined") {
        console.log("[v0] Firebase not available in server environment")
        return null
      }

      // Check if we're in v0 preview environment
      const hostname = window.location.hostname
      const isV0 = hostname.includes('v0.app') ||
        hostname.includes('v0-') ||
        hostname.includes('v0.run') ||
        hostname.includes('Next.js') ||
        hostname.includes('localhost') ||
        hostname.includes('127.0.0.1')

      if (isV0) {
        console.log("[v0] v0 preview environment detected, skipping Firebase initialization")
        return null
      }
      ...
      firestoreModule = { getDb: () => db, FIREBASE_COLLECTIONS, ...firestore }
      console.log("[v0] Firebase Firestore loaded and initialized successfully")
    } catch (error) {
      console.log("[v0] Firebase not available, using localStorage fallback:", error)
      firestoreModule = null
    }
  }
  return firestoreModule
...
async function loadFirebaseAuth() {
  if (!firebaseModule) {
    try {
      // Ensure we're in browser environment
      if (typeof window === "undefined") {
        console.log("[v0] Firebase Auth not available in server environment")
        return null
      }
      ...
      // Initialize auth to ensure it's ready
      const authInstance = getFirebaseAuth()

      firebaseModule = { getFirebaseAuth: () => authInstance, ...auth }
      console.log("[v0] Firebase Auth loaded and initialized successfully")
    } catch (error) {
      console.log("[v0] Firebase not available, using localStorage fallback:", error)
      firebaseModule = null
    }
...
export interface EmploymentApplication {
  ...
  coverLetter: string
  submittedAt: string
  // Adding status to EmploymentApplication for consistency with EnhancedEmploymentApplication
...
export interface JobPosition {
  ...
  workShift?: string
  workShiftEn?: string
  gender?: "ذكر" | "أنثى" | "لا يهم" // "ذكر" | "أنثى" | "لا يهم"
  genderEn?: "Male" | "Female" | "Doesn't matter" // "Male" | "Female" | "Doesn't matter"
...
export interface DynamicPage {
  ...
  blocks?: PageBlock[] // Deprecated - kept for backward compatibility
  blocksAr?: PageBlock[] // Arabic content blocks
  blocksEn?: PageBlock[] // English content blocks
  customCss?: string
  customJs?: string
  isPublished: boolean
  createdAt: string
...
export async function saveEmploymentApplication(
  ...
  const newApp: EmploymentApplication = {
    ...
    // Defaulting status to pending for new applications
    status: "pending",
  }
...
export async function getEmployees(): Promise<Employee[]> {
  ...
  console.log("[v0] Fetching employees from localStorage...")
  const employees = getFromLocalStorage<Employee[]>(COLLECTIONS.EMPLOYEES, [])
  console.log("[v0] Fetched", employees.length, "employees from localStorage")
  return employees

  // FIREBASE: Original Firestore employee fetching code
  /*
  try {
...
export async function getEmployeeById(id: string): Promise<Employee | null> {
  ...
  // FIREBASE: Original Firestore getDoc code
  /*
  try {
    const firestore = await loadFirestore()
    if (!firestore) {
...
export async function updateEmployee(id: string, updates: Partial<Employee>): Promise<void> {
  // LocalStorage implementation (active)
  console.log("[v0] Updating employee in localStorage:", id)
  const employees = getFromLocalStorage<Employee[]>(COLLECTIONS.EMPLOYEES, [])
  ...
  if (index !== -1) {
    ...
    console.log("[v0] Employee updated successfully in localStorage")
  }

  // FIREBASE: Original Firestore updateDoc code
  /*
  try {
    const firestore = await loadFirestore()
    if (!firestore) {
...
export async function saveEmployee(employee: Employee): Promise<void> {
  // LocalStorage implementation (active)
  console.log("[v0] Saving employee to localStorage:", employee.email)
  const employees = getFromLocalStorage<Employee[]>(COLLECTIONS.EMPLOYEES, [])
  ...
  saveToLocalStorage(COLLECTIONS.EMPLOYEES, employees)
  console.log("[v0] Employee saved successfully to localStorage")

  // FIREBASE: Original Firestore setDoc code
  /*
  try {
    const firestore = await loadFirestore()
    if (!firestore) {
...
export async function deleteEmployee(id: string): Promise<void> {
  // LocalStorage implementation (active)
  console.log("[v0] Deleting employee from localStorage:", id)
  const employees = getFromLocalStorage<Employee[]>(COLLECTIONS.EMPLOYEES, [])
  const filtered = employees.filter((emp) => emp.id !== id)
  saveToLocalStorage(COLLECTIONS.EMPLOYEES, filtered)
  console.log("[v0] Employee deleted successfully from localStorage")

  // FIREBASE: Original Firestore deleteDoc code
  /*
  try {
    const firestore = await loadFirestore()
    if (!firestore) {
...
export async function addEmployee(employee: Omit<Employee, "id" | "createdAt">): Promise<Employee> {
  ...
  console.log("[v0] Adding new employee to localStorage:", newEmployee.email)
  await saveEmployee(newEmployee)
  return newEmployee

  // FIREBASE: Original Firestore addDoc code
  /*
  try {
    const firestore = await loadFirestore()
    if (!firestore) {
...
export async function hasUsers(): Promise<boolean> {
  console.log("[v0] Checking if users exist in localStorage...")

  // LocalStorage implementation (active)
  const employees = getFromLocalStorage<Employee[]>(COLLECTIONS.EMPLOYEES, [])
  const hasLocalUsers = employees.length > 0
  console.log("[v0] LocalStorage users found:", hasLocalUsers)
  return hasLocalUsers

  // FIREBASE: Original Firebase implementation
  // try {
  //   if (typeof window === "undefined") {
  //     console.log("[v0] Server environment detected, using localStorage fallback")
  //     const employees = getFromLocalStorage<Employee[]>(COLLECTIONS.EMPLOYEES, [])
  ...
  //   const firestore = await loadFirestore()
  //   if (!firestore) {
  //     console.log("[v0] Firestore not available, checking localStorage")
  //     const employees = getFromLocalStorage<Employee[]>(COLLECTIONS.EMPLOYEES, [])
  //     return employees.length > 0
  //   }
  //   
  ...
  //   const db = firestore.getDb()
  //   
  //   console.log("[v0] Querying Firestore for employees...")
  //   const employeesRef = collection(db, firestore.FIREBASE_COLLECTIONS.EMPLOYEES)
  //   const snapshot = await getDocs(employeesRef)
  //   
  //   const hasFirebaseUsers = !snapshot.empty
  //   console.log("[v0] Firebase users found:", hasFirebaseUsers)
  //   
  //   return hasFirebaseUsers
  // } catch (error) {
  //   console.error("[v0] Error checking users in Firebase:", error)
  //   const employees = getFromLocalStorage<Employee[]>(COLLECTIONS.EMPLOYEES, [])
  //   const hasLocalUsers = employees.length > 0
  //   console.log("[v0] LocalStorage users found:", hasLocalUsers)
  //   return hasLocalUsers
...
export async function createFirstAdmin(
  ...
  try {
    console.log("[v0] Creating first admin in localStorage...")
    ...
    const newEmployee: Employee = {
      ...
      role: "admin",
      password: password, // Store password in localStorage for development
      permissions: {
    ...
    console.log("[v0] First admin created successfully in localStorage")
    return { success: true }
    ...
    console.error("[v0] Error creating first admin:", error)
    return {
      success: false,
...
export async function getDynamicPage(id: string): Promise<DynamicPage | null> {
  // LocalStorage implementation (active)
  console.log("[v0] Fetching dynamic page from localStorage:", id)
  const pages = getFromLocalStorage<DynamicPage[]>(COLLECTIONS.PAGES, [])
  const page = pages.find((p) => p.id === id)
  if (page) {
    console.log("[v0] Page found in localStorage:", id)
  } else {
    console.log("[v0] Page not found in localStorage:", id)
  }
  return page || null
  ...
  try {
    ...
    if (docSnap.exists()) {
      return docSnap.data() as DynamicPage
    } else {
      return null
    }
  } catch (error) {
...
export async function getDynamicPages(): Promise<DynamicPage[]> {
  // LocalStorage implementation (active)
  console.log("[v0] Fetching dynamic pages from localStorage...")
  const pages = getFromLocalStorage<DynamicPage[]>(COLLECTIONS.PAGES, [])
  console.log("[v0] Fetched", pages.length, "pages from localStorage")
  return pages

  // FIREBASE: Original Firestore getDocs code
  /*
  try {
    const firestore = await loadFirestore()
    if (!firestore) {
...
export async function saveDynamicPage(page: Omit<DynamicPage, "id" | "createdAt" | "updatedAt">): Promise<DynamicPage> {
  ...
  console.log("[v0] Saving dynamic page to localStorage...")
  const pages = getFromLocalStorage<DynamicPage[]>(COLLECTIONS.PAGES, [])
  pages.push(newPage)
...
export async function updateDynamicPage(id: string, updates: Partial<DynamicPage>): Promise<void> {
  // LocalStorage implementation (active)
  console.log("[v0] Updating dynamic page in localStorage:", id)
  const pages = getFromLocalStorage<DynamicPage[]>(COLLECTIONS.PAGES, [])
...
export async function deleteDynamicPage(id: string): Promise<void> {
  // LocalStorage implementation (active)
  console.log("[v0] Deleting dynamic page from localStorage:", id)
  const pages = getFromLocalStorage<DynamicPage[]>(COLLECTIONS.PAGES, [])
...
export async function getActivities(limit = 50): Promise<Activity[]> {
  // LocalStorage implementation (active)
  console.log("[v0] Fetching activities from localStorage...")
  const activities = getFromLocalStorage<Activity[]>(COLLECTIONS.ACTIVITIES, [])
  console.log("[v0] Fetched", activities.length, "activities from localStorage")
  return activities.slice(0, limit)

  // FIREBASE: Original Firestore query with orderBy code
  /*
  try {
    const firestore = await loadFirestore()
    if (!firestore) {
...
export async function logActivity(activity: Omit<Activity, "id" | "timestamp">): Promise<void> {
  // LocalStorage implementation (active)
  console.log("[v0] Logging activity to localStorage:", activity.action)
  const activities = getFromLocalStorage<Activity[]>(COLLECTIONS.ACTIVITIES, [])
  activities.unshift({
    ...activity,
    id: Date.now().toString(),
...
export async function getDynamicPageBySlug(slug: string): Promise<DynamicPage | null> {
  // LocalStorage implementation (active)
  console.log("[v0] Fetching page by slug from localStorage:", slug)
  const pages = getFromLocalStorage<DynamicPage[]>(COLLECTIONS.PAGES, [])
  ...
  // FIREBASE: Original Firestore query with where clause code
  /*
  try {
    const firestore = await loadFirestore()
    if (!firestore) {
