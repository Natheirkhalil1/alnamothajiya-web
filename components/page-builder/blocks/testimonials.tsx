        ...
        {(block.items || []).map((item) => (
          <div key={item.id} className="space-y-1 rounded-md border border-slate-200 bg-slate-50/60 p-2">
            <TextareaField
              ...
              onChange={(v) =>
                updateItems((items) =>
                  items.map((i) => (i.id === item.id ? (isAr ? { ...i, quote: v } : { ...i, quoteEn: v }) : i)),
                )
              }
              rows={3}
            />
            <InputField
              ...
              onChange={(v) =>
                updateItems((items) =>
                  items.map((i) => (i.id === item.id ? (isAr ? { ...i, author: v } : { ...i, authorEn: v }) : i)),
                )
              }
            />
            <InputField
              ...
              onChange={(v) =>
                updateItems((items) =>
                  items.map((i) =>
                    i.id === item.id ? (isAr ? { ...i, role: v || undefined } : { ...i, roleEn: v || undefined }) : i,
                  ),
                )
              }
            />
            ...
            {items.map((_, index) => (
              <button
                ...
                className={`h-3 w-3 rounded-full transition-all ${
                  index === currentIndex ? "w-8 bg-emerald-600" : "bg-slate-300 hover:bg-slate-400"
                }`}
